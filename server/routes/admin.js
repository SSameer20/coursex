...
adminRouter.post("/login", async (req, res) => {
  try {
    ...
    if (!email || !password)
      return res
        .status(400)
        .send({ message: "All fields are required to Login" });
    ...
    let passwordValidation =
      findUser.password === btoa(password) ? true : false;
    ...
    return res
      .status(201)
      .send({ message: "user logged successfully", token: token });
  } catch (error) {
...
adminRouter.post("/register", async (req, res) => {
  try {
    ...
    if (!email || !password || !firstname || !lastname) {
      return res
        .status(404)
        .send({ message: "All fields are required to register" });
    }
    ...
    if (findUser) {
      return res
        .status(404)
        .send({ message: "admin already exists with the same email" });
    }
...
adminRouter.post("/course/create", adminAuth, async (req, res) => {
  try {
    ...
    if (!title || !description)
      return res
        .status(404)
        .send({ message: "all details are required to create course" });

    const findCourse = await CourseModel.findOne({ title });
    if (findCourse)
      return res
        .status(404)
        .send({ message: "course with same title already exists" });
...
adminRouter.get("/course/view", adminAuth, async (req, res) => {
  try {
    ...
    if (findCourse.length > 0)
      return res
        .status(201)
        .send({ message: "course details", data: findCourse });
    return res
      .status(201)
      .send({ message: "no course available", data: findCourse });
  } catch (error) {
    return res.status(404).send({ message: error });
  }
});
