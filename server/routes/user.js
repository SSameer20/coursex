...
userRouter.get("/course/view", userAuth, async (req, res) => {
  try {
    const findCourse = await CourseModel.find().populate(
      "creatorId",
      "firstName secondName email"
    );
    if (findCourse.length > 0)
      return res.status(201).send({ message: "success", data: findCourse });
...
userRouter.get("/course/mycourse", userAuth, async (req, res) => {
  try {
    const { userId } = req.body;
    const findCourse = await PurchaseModel.find({ userId }).populate(
      "courseId",
    ...
    if (findCourse.length > 0)
      return res
        .status(201)
        .send({ message: "user courses", data: findCourse });
